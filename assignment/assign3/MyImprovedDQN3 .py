...
def run3():
    ...
    for i_episode in range(NUM_EPISODES):
        ...
        if np.array(condition[-10:]).mean() < 120 and i_episode>180:
            break
    ...
    for i_episode in range(NUM_EPISODES):
        ...
        for t in range (MAX_STEPS):
            ...
            if t == MAX_STEPS-1:
                ...
                r_data.append(ep_r)
                print("Episode %d terminated after %f MAX steps" % (i_episode, t))
        if np.array(condition[-10:]).mean() < 150 and i_episode>180:
            break
...
def run1():
    ...
    for i_episode in range(NUM_EPISODES):
        ...
        for t in range (MAX_STEPS):
            ...
            if t == MAX_STEPS-1:
                ...
                r_data.append(ep_r)
                print("Episode %d terminated after %f MAX steps" % (i_episode, t))
        if np.array(condition[-10:]).mean() > 2500 and i_episode>180:
            break
    ...
    EPSILON = 0
    result = []
    env = gym.wrappers.Monitor(env, './MyImprovedDQN/cartpole-vo', force=True)
