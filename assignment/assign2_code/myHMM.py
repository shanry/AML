...
class myHMM(object):
    ...
    def HMMfwd(self, a, b, o, pi):
        ...
        for t in range(1,T):     #modify xrange to range
            for i in range(N):   #modify xrange to range
                #my code here:
                alpha[i][t] = b[i][o[t-1]]*np.sum(alpha[:,t-1]*a[:,i])
                """
    ...
    def HMMbwd(self, a, b, o):
        ...
        for t in range(T-2,-1,-1):  #modify xrange to range
            for i in range(N):      #modify xrange to range
                #my code here:
    ...
    def HMMViterbi(self, a, b, o, pi):
        ...
        N = np.shape(b)[0]  #状态数
        T = np.shape(o)[0]  #观测序列长度
        
        path = np.zeros(T)
        delta = np.zeros((N,T))  #T1
        phi = np.zeros((N,T))    #T2
        ...
        #dynamic progranmming for T1 and T2
        for i in range (1,T) :
            for j in range (N) :
                possible = delta[:,i-1]*a[:,j]*b[j][o[i]]  #broadcast of python
                delta[j][i] = np.max(possible)
                phi [j][i] = np.argmax (possible)
...
def parseStockPrices(from_date, to_date, symbol):
    ...
    np_hist_prices = np_hist_prices[::-1]
